...
class PermissionBaseFilter(BaseFilterSet):
    ...
    def filter_user(self, queryset):
        ...
        asset_perm_ids_1 = AssetPermission.objects.filter(users=user).distinct().values_list('id', flat=True)
        asset_perm_ids_2 = AssetPermission.objects.filter(user_groups__in=groups).distinct().values_list('id', flat=True)

        asset_perm_ids = {*asset_perm_ids_1, *asset_perm_ids_2}
